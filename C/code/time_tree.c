...
void get_leaf_for_insert(float key, char* leaf){
	...
	while(current_time_node->is_leaf == FALSE){
		...
		while(end > start) {
			mid = (end - start) / 2;
			if(current_time_node->keys[mid] > key) {
				end = mid - 1;
			} else if(current_time_node->keys[mid] < key) {
				start = mid + 1;
			} else {
				end = mid;
			}
		...
		if(current_time_node->keys[start] != key){
			start = current_time_node->num_filled;
		}
...
void leaf_insert(char* leaf_filename, float key, char* value){
	...
	while(end > start) {
		mid = (end - start) / 2;
		if(leaf_time_node->keys[mid] > key) {
			end = mid - 1;
		} else if(leaf_time_node->keys[mid] < key) {
			start = mid + 1;
		} else {
			end = mid;
		}
	...
	if(leaf_time_node->keys[start] != key){
		start = leaf_time_node->num_filled;
	}
