...
int main(int argc, char **argv)
{
            ...
            if(all_messages[i].time_id == time_current->time_id){
                ...
                while(nebraskaUser_current != NULL){
                    ...
                    if(all_messages[i].user_id == nebraskaUser_current->user_id){
                        ...
                        //remove user_id from userList so we don't count twice
                        
                        //First user in the list was from Nebraska
                        if(previous_user == NULL){
                            nebraskaUser_head = nebraskaUser_head->next;

                        } else{
                            previous_user->next = nebraskaUser_current->next;
                        }
                        

                    }
                    previous_user = nebraskaUser_current;
    ...
    printf("\nThere are %d Nebraska users whos sent messages between 8 and 9.", user_count);
